...
var FlappyBird = function() {
    ...
    this.update = function() {
        if(gameState === 1 && started) {
            ...
            if(abs(this.height) >= 1.0) {
                this.heightIncrement *= -0.9;
            }
        }
        
        if(this.dead) {
            this.descentSpeed = min(32, this.descentSpeed + 1.0);
...
draw = function() {
    ...
    if(gameState === 2) {
        ...
        if(FlappiestBird.y >= 325 && deathTicks >= 15) {
            ...
            text(score, 305, 200);
            text(score, 305, 250);
            text("HA, YOU'RE DEAD STUPID BIRD!",316.5,270);
        }
    }
};
